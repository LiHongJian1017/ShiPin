...
<body>
	<div id="previewWrapper">
		<div class="preview_header">
			...
			<a title="" class="inline cboxElement"
		...
		<div class="preview_content">
			<div class="profile_box" id="basicInfo">
				<h2>基本信息</h2>
				<div class="basicShow">
					<span>${jianli.name} | ${jianli.sex} | <c:if
							test="${jianli.education =='0'}">
						其他 |
					...
					<br> ${jianli.schoolname} | ${jianli.specialty}<br>
					 ${jianli.phone} | ${jianli.email}<br> 
					<c:if test="${not empty jianli.workstatus}">
								${jianli.workstatus}
					</c:if>
					</span>
			...
			<div class="profile_box" id="expectJob">
				...
				<div class="experienceShow">
					...
					<c:if test="${empty jianli.certification}">
						无资格认证
					</c:if>
					<c:if test="${not empty jianli.certification}">
						${jianli.certification}
					</c:if>
					</div>
				</div>
				<!--end .experienceShow-->
			...
			<div class="profile_box" id="expectJob">
				...
				<c:if test="${not empty jianli.positionnature}">
					${jianli.positionnature}，
				</c:if>
				<c:if test="${not empty jianli.salarymin}">
					月薪 ${jianli.salarymin}K
				</c:if>
				<c:if test="${not empty jianli.salarymax}">
					-${jianli.salarymax}K，
				</c:if>
				<c:if test="${not empty jianli.hopepositionname}">
					${jianli.hopepositionname}
				</c:if>
				</div>
				<!--end .expectShow-->
			</div>
			...
			<div class="profile_box" id="educationalBackground">
				...
				<div class="educationalShow">
					<ul class="elist clearfix">
						<li class="clear">
						<span class="c9">
						<c:if test="${not empty jianli.yearstart && not empty jianli.yearend}">
							${jianli.yearstart}-${jianli.yearend}
						</c:if>
						</span>
							<div>
								<h3>
